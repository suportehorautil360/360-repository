...
import {
  ...
  notasFiscaisApi,
  type NotaFiscalOficinaListItem,
  type NotasFiscaisOficinasResumo,
...
const RESUMO_VAZIO: NotasFiscaisOficinasResumo = {
  totalNotas: 0,
  valorTotal: 0,
  pendentes: 0,
  aprovadas: 0,
...
export function NotasFiscaisOficinasSection({
  prefeituraId,
}: {
  prefeituraId: string;
}) {
  const [rows, setRows] = useState<NotaFiscalOficinaListItem[]>([]);
  const [resumo, setResumo] = useState<NotasFiscaisOficinasResumo>(RESUMO_VAZIO);
  ...
  const carregar = useCallback(async () => {
    ...
    setErro(null);
    try {
      const resp = await notasFiscaisApi.listarOficinasPorPrefeitura(
        prefeituraId,
        {
          busca,
          status,
  ...
  return (
    <section className="nf-page">
      <div className="nf-wrap">
        <h1 className="nf-title">Notas Fiscais de Oficinas</h1>
