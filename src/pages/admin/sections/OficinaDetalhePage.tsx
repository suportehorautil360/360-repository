...
import { useOficinaDetalhe } from "./oficina-detalhe/use-oficina-detalhe";
...
export function OficinaDetalhePage() {
  ...
  const subtitulo = detalhe?.cidadeUf?.trim() || "";
  ...
  if (!oficinaId) {
    return (
      <section className="mx-auto max-w-6xl px-1 py-6">
        <p className="text-red-400">Identificador da oficina inválido.</p>
    ...
    return (
      <section className="mx-auto max-w-6xl px-1 py-6">
        <p className="text-red-400">
          {erroCarregamento ?? "Oficina não encontrada."}
        </p>
        <Button asChild variant="secondary" className="mt-4">
    ...
    <section className="mx-auto max-w-6xl space-y-6 px-1 pb-10">
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="grid size-12 place-items-center rounded-2xl bg-sky-500/15 text-sky-400">
          ...
          <Badge variant={detalhe.ativo !== false ? "posto" : "local"}>
            {detalhe.ativo !== false ? "Ativa" : "Suspensa"}
          </Badge>
        </div>
      </div>
          ...
          <CardContent className="space-y-3">
            ...
            {credencialNova.email ? (
              <CredencialItem
                ...
                valor={credencialNova.email}
              />
            ) : null}
            <CredencialItem rotulo="Senha" valor={credencialNova.senha} />
            <p className="text-xs leading-relaxed text-emerald-200/80">
              A senha não fica visível depois de salva. O operador entra no app
              da oficina com o e-mail (ou usuário) e esta senha.
            </p>
          </CardContent>
        </Card>
      ) : null}
      ...
      <Tabs defaultValue="dados" className="gap-6">
        ...
        <TabsContent value="dados" className="mt-0">
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <InfoCard titulo="Atuação">
              <InfoChips rotulo="Linhas" itens={detalhe.linhasAtuacao} />
              <InfoChips rotulo="Categorias" itens={detalhe.categoriasServico} />
              <InfoRow rotulo="Especificações" valor={detalhe.especificacoes} />
            </InfoCard>
        ...
        <TabsContent value="acessos" className="mt-0">
          <div className="grid gap-6 lg:grid-cols-5">
            <Card className="lg:col-span-3">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <KeyRound className="size-4 text-sky-400" aria-hidden />
                  Equipe com acesso
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="mb-4 text-sm leading-relaxed text-slate-400">
                  Cada operador entra no app da oficina com{" "}
                          ...
                          <TableRow key={a.id}>
                            <TableCell>
                              <div className="flex items-center gap-3">
                                <span className="grid size-9 place-items-center rounded-full bg-sky-500/15 text-xs font-bold text-sky-300">
            ...
            <Card className="border-sky-500/20 bg-sky-500/[0.04] lg:col-span-2 lg:self-start">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent>
                <form onSubmit={handleCriar} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="oficina-nome">Nome completo</Label>
                    <Input
                      id="oficina-nome"
                      value={nome}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="oficina-email">E-mail de acesso</Label>
                    <Input
                      id="oficina-email"
                      type="email"
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      placeholder="operador@oficina.com.br"
                      autoComplete="email"
                      required
                    />
                    <p className="text-xs text-slate-500">
                      Usado para login no app da oficina (postoapp).
                    </p>
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="oficina-senha">Senha inicial</Label>
                    <Input
                      id="oficina-senha"
                      type="password"
                      value={senha}
      ...
      >
        <DialogContent className="border-white/10 bg-[#0e1424] text-slate-100">
          ...
          <div className="space-y-2">
            <Label htmlFor="oficina-nova-senha">Nova senha</Label>
            <Input
              id="oficina-nova-senha"
              type="password"
              value={novaSenha}
