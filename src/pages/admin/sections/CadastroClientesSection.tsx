...
function labelModalidade(v: string | undefined): string {
  if (!v) return "—";
  if (v === "contrato_privado_locacao")
    return "Contrato comercial (sem licitação)";
...
export function CadastroClientesSection() {
  const { listarClientes, adicionarCliente, removerCliente } = useClientes();
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    const r = await adicionarCliente(payload);
    setLoading(false);
    if (!r.ok) {
      setMsgTexto("err", r.message ?? "Não foi possível salvar o cliente.");
      return;
    }
  ...
  async function handleRemover(id: string, nome: string) {
    ...
    setLoading(true);
    const r = await removerCliente(id);
    setLoading(false);
    if (!r.ok) {
      setMsgTexto("err", r.message ?? "Não foi possível remover o contrato.");
      return;
    }
    ...
    <section id="acessos" className="aba-conteudo ativa">
      ...
      <article className="card">
        ...
        <form id="formPrefeituraContrato" onSubmit={handleSubmit}>
          ...
          <div className="contrato-secao">
            <div
              id="cadastroContratanteTitulo"
              className="contrato-secao-titulo"
            >
              {isLoc
            ...
            <div className="row-2">
              <div>
                ...
                <input
                  ...
                  required
                  placeholder={
                    isLoc ? "Ex.: LOC-FROTA-2026-018" : "Ex.: 012/2026"
                  }
                  value={form.numero}
          ...
          <div className="contrato-secao">
            <div id="ctr-titulo-objeto" className="contrato-secao-titulo">
              {isLoc
                ? "Escopo para locação / frota e software"
            ...
            <tbody id="tabelaPrefeituras">
              {clientes.length === 0 ? (
                ...
                clientes.map((p) => {
                  ...
                  const tipo =
                    p.tipoCliente === "locacao" ? "Locação" : "Prefeitura";
                  const modalidade =
                    p.tipoCliente === "locacao"
                      ? labelModalidade(
                          c?.modalidade || "contrato_privado_locacao",
                        )
