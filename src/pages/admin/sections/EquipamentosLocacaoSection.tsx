...
export function EquipamentosLocacaoSection() {
  ...
  // Carrega lista do Firestore quando muda o cliente
  useEffect(() => {
    if (prefId) void equip.carregar(prefId);
  ...
  async function salvarEquipamentoManual(e: FormEvent) {
    e.preventDefault();
    if (!prefId) {
      setEqMsgManual({ texto: "Selecione o cliente.", cor: COR_ERRO });
      return;
    }
    const resultado = await equip.adicionar({
      prefeituraId: prefId,
      descricao: eqForm.descricao,
      marca: eqForm.marca,
      modelo: eqForm.modelo,
      chassis: eqForm.chassis,
      linha: eqForm.linha,
      obra: eqForm.obra,
    });
    if (!resultado.ok) {
      setEqMsgManual({ texto: resultado.message, cor: COR_ERRO });
      return;
    }
  ...
  async function importarPlanilha() {
    ...
    if (!prefId) {
      setEqMsgImport({ texto: "Selecione o cliente.", cor: COR_ERRO });
      return;
    }
    const { adicionados } = await equip.importarTexto(prefId, eqPaste);
    if (adicionados === 0) {
      setEqMsgImport({
        texto:
  ...
  async function lerArquivoEquipamentos(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const { adicionados } = await equip.importarArquivo(prefId, arquivo);
      if (adicionados === 0) {
        setEqMsgImport({
          ...
          >
            <div>
              ...
              <input
                ...
                onChange={(e) => atualizarEqForm("descricao", e.target.value)}
              />
            </div>
            <div>
