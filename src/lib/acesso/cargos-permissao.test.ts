...
import {
  ...
  podeAcessarGrupo,
  resolverPorCargo,
} from "./cargos-permissao";

describe("normalizarCargo", () => {
  it("mapeia aliases legados para nomes novos", () => {
    expect(normalizarCargo("Operador")).toBe("operador de manutenção");
    expect(normalizarCargo("Supervisor")).toBe("supervisor de rh");
  });
});

describe("podeAcessarGrupo — gating por cargo", () => {
  it("admin vê todos os grupos", () => {
    const admin = { perfil: "admin", cargo: "Operador de Manutenção" };
  ...
  it("Operador de Manutenção e Mecânico → só Manutenção", () => {
    for (const cargo of [
  ...
  it("Supervisor de RH (e alias Supervisor) → só Pessoas / RH", () => {
    for (const cargo of ["Supervisor de RH", "Supervisor"]) {
      const u = { perfil: "gestor", cargo };
  ...
  it("mapa da API sobrescreve o default", () => {
    const u = { perfil: "gestor", cargo: "Motorista" };
    const mapa = resolverPorCargo({
      motorista: [GRUPO_MANUTENCAO],
    });
    expect(podeAcessarGrupo(GRUPO_MANUTENCAO, u, mapa)).toBe(true);
    expect(podeAcessarGrupo(GRUPO_FROTA, u, mapa)).toBe(false);
  });

  it("defaults incluem aliases legados", () => {
    expect(DEFAULT_POR_CARGO.operador).toEqual([GRUPO_MANUTENCAO]);
    expect(DEFAULT_POR_CARGO.supervisor).toEqual([GRUPO_PESSOAS]);
  });
});
